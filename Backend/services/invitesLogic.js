...
export const updateByInviteResponse = async (userId, inviteId, answer) => {
  try {
    ...
    const invite = await InvitesModel.findByIdAndDelete(inviteId).select("event email");
    ...
    if(answer) {
      ...
      if(user.matchedCount === 0) throw new DataNotFoundError("couldnt find user with email like in the invite");
      ...
      if(event.matchedCount === 0) throw new DataNotFoundError("couldnt find event with this eventId and collaborator email like in the invite"); 
    }
    else {
      const event = await eventModel.updateOne(
        { _id: invite.event },
